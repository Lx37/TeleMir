...
class GetFeatures():
    def __init__(self,stream_in, name = 'test'):
        ...
        #blink_proj = np.load('./analyses/ICA_proj.npy')
        blink_proj = np.load(sys.path[0] + '/analyses/ICA_proj.npy')
        self.blink_invProj = np.linalg.inv(blink_proj)
        self.n_blink_comp = 2
        self.last_blink = 0
        self.blink_threshold = 0.05#5

        sacc_proj = np.load(sys.path[0] + '/analyses/ICA_proj_test_saccade.npy')
        self.sacc_invProj = np.linalg.inv(sacc_proj)
        self.n_sacc_comp = 1
        self.lastSacc = 0
        self.sacc_threshold = 0.04

    def extract_TCL(self, data):

        #print data
        self.getFeat(data)
        blink_feat, mean_blink_comp, sacc_feat, mean_sacc_comp = self.is_blink_sacc(data)
        ...
        pAlphaO12 = (self.pows[11,2] + self.pows[12,2])  #was /2
        ...
        contrib_alpha_O1O2 = pAlphaO12*100/total_power_O1O2
        self.contrib_alpha_cumul_O1O2.append(contrib_alpha_O1O2)
        contrib_alpha_cumul_O1O2 = np.sum(self.contrib_alpha_cumul_O1O2)

        #frontal_theta
        pFrontalTheta = self.pows[0,1] + self.pows[1,1] + self.pows[3,1] + self.pows[8,1]
        total_power_Frontal = np.sum(self.pows[0,:], axis=0) + np.sum(self.pows[1,:], axis=0) + np.sum(self.pows[3,:], axis=0) + np.sum(self.pows[8,:], axis=0)
        frontal_theta = pFrontalTheta*100/total_power_Frontal
        ...
        if (contrib_alpha_smooth > 0.1 and contrib_alpha_smooth < 0.4):
            contrib_alpha_smooth_norm = int((contrib_alpha_smooth - 0.1)*600)  # etrange * 50, *33
        else:
        ...
        if (contrib_beta_smooth > 0.06 and contrib_beta_smooth < 0.3):
            contrib_beta_smooth_norm = int((contrib_beta_smooth - 0.06)*500)
        else:
        ...
        if (contrib_teta_smooth > 0.1 and contrib_teta_smooth < 0.4):
            contrib_teta_smooth_norm = int((contrib_teta_smooth - 0.06)*500)
        else:
        ...
        if (contrib_mu_smooth > 0.1 and contrib_mu_smooth < 0.4):
            contrib_mu_smooth_norm = int((contrib_mu_smooth - 0.1)*600)
        else:
        ...
        R_engage = bandsAv[3]/(bandsAv[1]+bandsAv[2])  # beta / (alpha + theta)
        ...
        features = np.array([contrib_alpha_smooth_norm, blink_feat, crisp_feat,
                            contrib_beta_smooth_norm, contrib_teta_smooth_norm, contrib_mu_smooth_norm,
                            mean_blink_comp, sacc_feat, mean_sacc_comp,
                            contrib_alpha_O1O2, R_engage, frontal_theta])


        return features

    def is_blink_sacc(self, head):
        ...
        sacc_comp = np.dot( self.sacc_invProj[self.n_sacc_comp], data_rect[:,-10])


        mean_blink_comp = np.mean(blink_comp)
        if mean_blink_comp > self.blink_threshold and self.lastBlink < self.blink_threshold:
            isBlink = 1
        else:
            isBlink = 0
        ...
        #~ else:
            #~ isBlink = 0
        mean_sacc_comp = np.mean(sacc_comp)
        if mean_sacc_comp > self.sacc_threshold and self.lastSacc < self.sacc_threshold:
            isSacc = 1
        else:
            isSacc = 0
        self.lastSacc = np.mean(sacc_comp)

        return isBlink, mean_blink_comp, isSacc, mean_sacc_comp
